"""
This file shows application of Bagging model to predict 'ECs adsorption capacity on Biochar materials'
"""
...
ba_run = ensemble.BaggingRegressor(random_state=0)
ba_run.get_params()

def bo_params_ba(n_estimators):
    params = {'n_estimators': int(n_estimators)}
    reg = ensemble.BaggingRegressor(random_state=0,
...
ba_BO = BayesianOptimization(bo_params_ba, {'n_estimators': (1, 250)},
                             random_state=1)
results = ba_BO.maximize(n_iter=200, init_points=20,acq='ei')
params = ba_BO.max['params']
...
ba_run = ensemble.BaggingRegressor(random_state=0,
                                   n_estimators=params['n_estimators'])
ba_run.fit(X_train, y_train)

## 06. Model evaluation
    # train
train_predict = ba_run.predict(X_train)
print("The training RMSE of the Bagging model is :\t", np.sqrt(mean_squared_error(train_predict,y_train)))
print("The training R^2 of the Bagging model is :\t", r2_score(train_predict,y_train))
print("The training MAE of the Bagging model is :\t", mean_absolute_error(train_predict,y_train))
    # validation
test_predict = ba_run.predict(X_test)
print("The test RMSE of the Bagging model is :\t",  np.sqrt(mean_squared_error(test_predict,y_test)))
print("The test R^2 of the Bagging model is :\t",  r2_score(test_predict,y_test))
print("The test MAE of the Bagging model is :\t",  mean_absolute_error(test_predict,y_test))
