"""
This file shows application of HistGradientBoosting model to predict 'ECs adsorption capacity on Biochar materials'
"""
...
hgb_run = ensemble.HistGradientBoostingRegressor(random_state=0)
hgb_run.get_params()

def bo_params_hgb(max_depth, min_samples_leaf, max_leaf_nodes, learning_rate):
    params = {'max_depth': int(max_depth),
              'min_samples_leaf': int(min_samples_leaf),
              'max_leaf_nodes': int(max_leaf_nodes),
              'learning_rate': learning_rate}
    reg = ensemble.HistGradientBoostingRegressor(random_state=0,
...
hgb_BO = BayesianOptimization(bo_params_hgb, {'max_depth': (60,100),
                                              'min_samples_leaf': (1,4),
                                              'max_leaf_nodes': (2,50),
                                              'learning_rate': (0.001, 1.0)},
                              random_state=1)
results = hgb_BO.maximize(n_iter=200, init_points=20,acq='ei')
params = hgb_BO.max['params']
...
hgb_run.fit(X_train, y_train)

## 06. Model evaluation
    # train
train_predict = hgb_run.predict(X_train)
print("The training RMSE of the HistGradientBoosting model is :\t", np.sqrt(mean_squared_error(train_predict,y_train)))
print("The training R^2 of the HistGradientBoosting model is :\t", r2_score(train_predict,y_train))
print("The training MAE of the HistGradientBoosting model is :\t", mean_absolute_error(train_predict,y_train))
    # validation
test_predict = hgb_run.predict(X_test)
print("The test RMSE of the HistGradientBoosting model is :\t",  np.sqrt(mean_squared_error(test_predict,y_test)))
print("The test R^2 of the HistGradientBoosting model is :\t",  r2_score(test_predict,y_test))
print("The test MAE of the HistGradientBoosting model is :\t",  mean_absolute_error(test_predict,y_test))
