"""
This file shows application of ExtraTree model to predict 'ECs adsorption capacity on Biochar materials'
"""
...
et_run = ensemble.ExtraTreesRegressor(random_state=0)
et_run.get_params()

def bo_params_et(max_depth, min_samples_leaf, min_samples_split, n_estimators):
    params = {'max_depth': int(max_depth),
              'min_samples_leaf': int(min_samples_leaf),
              'min_samples_split': int(min_samples_split),
              'n_estimators': int(n_estimators)}
    reg = ensemble.ExtraTreesRegressor(random_state=0,
...
et_BO = BayesianOptimization(bo_params_et, {'max_depth': (60,100),
                                            'min_samples_leaf': (1,4),
                                            'min_samples_split': (2,10),
                                            'n_estimators': (100, 250)},
                              random_state=1)
results = et_BO.maximize(n_iter=200, init_points=20,acq='ei')
params = et_BO.max['params']
...
et_run.fit(X_train, y_train)

## 06. Model evaluation
    # train
train_predict = et_run.predict(X_train)
print("The training RMSE of the ExtraTrees model is :\t", np.sqrt(mean_squared_error(train_predict,y_train)))
print("The training R^2 of the ExtraTrees model is :\t", r2_score(train_predict,y_train))
print("The training MAE of the ExtraTrees model is :\t", mean_absolute_error(train_predict,y_train))
    # validation
test_predict = et_run.predict(X_test)
print("The test RMSE of the ExtraTrees model is :\t",  np.sqrt(mean_squared_error(test_predict,y_test)))
print("The test R^2 of the ExtraTrees model is :\t",  r2_score(test_predict,y_test))
print("The test MAE of the ExtraTrees model is :\t",  mean_absolute_error(test_predict,y_test))
