"""
This file shows application of Decision Tree model to predict 'ECs adsorption capacity on Biochar materials'
"""
...
dt_run = tree.DecisionTreeRegressor(random_state=0)
dt_run.get_params()

def bo_params_dt(max_depth, min_samples_leaf, min_samples_split):
...
dt_BO = BayesianOptimization(bo_params_dt, {'max_depth': (60,100),
                                            'min_samples_leaf': (1,4),
                                            'min_samples_split': (2,10)},
                              random_state=1)
results = dt_BO.maximize(n_iter=200, init_points=20,acq='ei')
...
dt_run = tree.DecisionTreeRegressor(random_state=0,
                                    ...
                                    min_samples_leaf=params['min_samples_leaf'],
                                    min_samples_split=params['min_samples_split'])
dt_run.fit(X_train, y_train)

## 06. Model evaluation
    # train
train_predict = dt_run.predict(X_train)
print("The training RMSE of the Decision tree model is :\t", np.sqrt(mean_squared_error(train_predict,y_train)))
print("The training R^2 of the Decision tree model is :\t", r2_score(train_predict,y_train))
print("The training MAE of the Decision tree model is :\t", mean_absolute_error(train_predict,y_train))
    # validation
test_predict = dt_run.predict(X_test)
print("The test RMSE of the Decision tree model is :\t",  np.sqrt(mean_squared_error(test_predict,y_test)))
print("The test R^2 of the Decision tree model is :\t",  r2_score(test_predict,y_test))
print("The test MAE of the Decision tree model is :\t",  mean_absolute_error(test_predict,y_test))
